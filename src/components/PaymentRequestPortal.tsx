...
export default function PaymentRequestPortal({ 
  ...
  return (
    <div style={{ padding: '24px', color: t.text }}>
      ...
      {/* Filters Bar */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', marginBottom: '24px', background: t.card, padding: '20px', borderRadius: '20px', border: `1px solid ${t.border}`, alignItems: 'center' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <label style={labelStyle}>Department</label>
          <select 
            value={filters.department}
            onChange={(e) => setFilters({...filters, department: e.target.value})}
            style={{ ...inputStyle(t), width: '180px' }}
          >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <input 
              ...
              value={filters.startDate}
              onChange={(e) => setFilters({...filters, startDate: e.target.value})}
              style={{ ...inputStyle(t), width: '150px' }}
            />
            <span style={{ color: t.textMuted, fontWeight: 700 }}>to</span>
            <input 
              type="date" 
        ...
        <button 
          ...
          style={{ alignSelf: 'flex-end', height: '42px', padding: '0 20px', borderRadius: '12px', background: 'transparent', border: `1px solid ${t.border}`, color: t.textMuted, fontSize: '0.8125rem', fontWeight: 700, cursor: 'pointer' }}
        >
          Reset Filters
        </button>
      </div>
          ...
          <div style={{ ...modalContentStyle, background: t.card, maxWidth: '650px' }}>
            ...
            <form onSubmit={handleSubmit} style={{ padding: '28px', maxHeight: '75vh', overflowY: 'auto' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                ...
                <div style={{ gridColumn: 'span 2' }}>
                  <FileUpload 
                    label={formData.isNewVendor ? "Supportings & KYC Documents" : "Supportings Documents"} 
                    files={formData.supportings} 
                    onFilesChange={files => setFormData({...formData, supportings: files})} 
                    t={t} 
                  />
